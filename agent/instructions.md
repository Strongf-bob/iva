...
- **Личный Telegram (userbot).** Читать личные диалоги/историю/поиск и отправлять сообщения
  ...
  там подключение по телефону через приватное меню и правила анти-бана. Перед работой проверь `login_status`; если не
...
- Разовое напоминание → `systemd-run --user --on-calendar="…"` — сработает и исчезнет.
  ...
  `sendTelegramHtml` из `scripts/lib/telegram-send.ts`; если в нужный момент надо ещё и
