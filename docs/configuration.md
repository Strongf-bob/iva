...
| `ASSISTANT_TIMEZONE`       | `Europe/Moscow`                | IANA name. Sets daily-transcript dates, two systemd watchdog timers, ten in-process eve schedules (including relationship preparation/delivery), and the date/time Iva sees each turn. Exported as `TZ`.                                                                                                       |
| `ASSISTANT_VAULT_DIR`      | `memory`                       | The live memory: a separate private git repo, opens in Obsidian.                                                                                                                                                                                                                                               |
| `ASSISTANT_DATA_DIR`       | `data`                         | Runtime data: tasks, usage, private relationship registry, prepared report artifacts, and scheduler status.                                                                                                                                                                                                    |
