...
![Недоверенный ввод из Telegram, веба и почты проходит гейт безопасности: заражённые сообщения падают в отсев, до vault доходит только чистый контекст](../../assets/iva-security-gate.webp)
...
- 🔐 **Ключи и вход в userbot** - учётные данные остаются на сервере в приватных runtime-файлах. Телефон, API-ключи и пароль 2FA принимаются детерминированными секретными шагами меню: текст удаляется до обработки и не уходит модели или в daily-лог. В container production отдельный onboarding-bearer смонтирован только в poller и sidecar, но не в контейнер модели. Данные всё равно проходят через транспорт Telegram-бота и могут кратко находиться во входной очереди, поэтому удаление не равно стиранию.
