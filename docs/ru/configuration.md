...
| --------------------------- | ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `MODEL_PROVIDER`            | `opencode`        | `ollama` (Ollama Cloud), `opencode` (OpenCode Go), `openrouter` (OpenRouter) или `codex` (подписка OpenAI ChatGPT).                                                                                    |
...
## Проактивные обзоры (только владелец)

По умолчанию проактивные обзоры выключены. Чтобы включить их для владельца, добавьте локальную настройку в `data/settings.json`:
...
Расписание просыпается каждые пять минут, готовит дневной брифинг в 05:00 и недельный обзор по понедельникам в 05:15, фиксирует подготовленные версии в 07:55 и доставляет их в 08:00 по `Europe/Moscow`. В понедельник дневной и недельный отчёты объединяются в одно сообщение. Пропущенный дневной запуск восстанавливается в течение 12 часов, недельный — в течение 72 часов. Версии, попытки, квитанции доставки, интервалы подавления и подтверждения хранятся в приватной директории `data/proactive-reviews/`.

Рантайм читает только узкие снимки провайдеров из `data/proactive-reviews/sources/{unified-inbox,crm,calendar,tasks}.json`; сборщики источников подключаются отдельно. Каждый файл — JSON-массив строгих нормализованных объектов с `id`, `title`, хотя бы одной ссылкой `evidence` и необязательными `summary`, `occurredAt` или `dueAt` в миллисекундах Unix. Отсутствующий файл означает пустой источник; повреждённый файл или симлинк отклоняется.

Отчёты и срочные оповещения получает только приватный bot-чат владельца. Оповещения `high` ждут окончания тихого времени 22:00–08:00 и подавляются на шесть часов; `critical` могут обходить тихое время и подавляются на час. Найденные обязательства остаются внутренними, пока владелец не нажмёт **Create Google Task**. Создание идемпотентно, а **Dismiss** лишь сохраняет решение и не меняет данные Google. Личный Telegram userbot остаётся только для чтения и в этом сценарии не используется.
...
| `ASSISTANT_TIMEZONE`       | `Europe/Moscow`                | Имя из базы IANA. Задаёт даты дневных транскриптов, два systemd watchdog-таймера, шесть внутрипроцессных eve schedules и дату/время, которые Iva видит на каждом ходе. Экспортируется как `TZ`.                                                                                                      |
| `ASSISTANT_VAULT_DIR`      | `memory`                       | Живая память: отдельный приватный git-репозиторий, открывается в Obsidian.                                                                                                                                                                                                                           |
