...
| `ASSISTANT_TIMEZONE`       | `Europe/Moscow`                | Имя из базы IANA. Задаёт даты дневных транскриптов, два systemd watchdog-таймера, пять внутрипроцессных eve schedules и дату/время, которые Iva видит на каждом ходе. Экспортируется как `TZ`.                                                                                                       |
