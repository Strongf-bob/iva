...
- 👥 **Один бот теперь безопасно разделён между пользователями** - до 10 взаимно недоверенных людей работают только в личных чатах, каждый через отдельный Eve-воркер со своими vault, историей, настройками, расписаниями, Google-подключением и учётом расхода. Локальная CLI управляет добавлением, блокировкой, лимитами и карантином; явная проверяемая миграция переносит прежнего владельца с rollback-бэкапом. Общие провайдеры оплачивает владелец сервера, userbot остаётся только у владельца, а модельный shell выключен у всех воркеров.
