...
- 📬 Сообщения сохраняются даже после перезапуска.
- ⚡ Статус «работаю» появляется быстрее.
- 💬 Ива понимает, когда вы цитируете предыдущее сообщение.
- ✂️ Ива предупреждает, если длинный текст пришлось сократить.
- ↩️ При ошибке возвращаются предыдущие настройки провайдера и модели.
- 📱 В меню видно, подключён ли личный Telegram. Если подключение не работает, Ива показывает причину.
- ⏭️ Если написать Иве во время ответа, новое сообщение обработается следом. Отправлять ещё одно сообщение не нужно.
- 🧹 Если команда зависла, Ива останавливает её и продолжает работать.
- 🛑 Ива не перезапускает себя во время ответа.
- 🤖 Добавлена модель Kimi K3.
- 🖼️ Исправлено распознавание изображений.
- 🔒 Закрыты найденные уязвимости.

#### v0.3.4

- 🧠 Уровень размышлений можно настроить для выбранной модели.
- 🆕 `/new` очищает только текущий чат или тему.
- 🔄 `/restart` очищает текущий чат или тему и перезапускает Иву.
- 📝 При обновлении памяти старые записи сохраняются.
- 🔐 Личные файлы и настройки стали лучше защищены.
