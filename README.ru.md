...
- **Живые ответы** - таблицы, чек-листы, сворачиваемые блоки и формулы рендерятся прямо в Telegram (rich messages Bot API 10.2); обычное форматирование идёт проверенным путём, с мягким откатом при сбое.
...
- **Отношения со ссылками на источники** - приватные досье контактов, договорённости, дни рождения и следующие шаги остаются привязаны к исходным сообщениям; отчёты приходят только владельцу в личный чат с ботом.
- **Единый входящий поток** - ограниченный read-only сбор Gmail, Календаря и личного Telegram превращает свежие факты в приоритеты и подготовку к встречам, но сам ничего не отправляет.
- **Проактивные обзоры** - включаемые отдельно дневные и недельные брифинги фиксируют версии отчётов, убирают дубли алертов и требуют подтверждения владельца для каждой Google Task.
- **Брифы шефа штаба** - `/brief`, `/brief <человек>` и `/weekly` превращают задачи и актуальные источники vault в проверяемые приоритеты и подготовку к встречам без внешних действий.
- **Память о людях** - `/person <человек>` показывает актуальные знания со ссылками на источники в native rich-карточке; приватное owner-only меню безопасно добавляет или явно исправляет один факт без дублей контактов и потери History.
...
- **Google Workspace** - черновики Gmail (без отправки), события Календаря без участников, файлы Drive, Таблицы, Документы и подтверждённые владельцем Задачи через CLI `gws`; ставится сам, а подключение ключа проходите по шагам прямо в переписке.
- **Скиллы и MCP** - один файл, чтобы добавить процедуру или подключить MCP-сервер; ключи остаются в `.env`.
- **Личный Telegram - userbot (бета)** - читать и искать в _вашем собственном_ аккаунте, а не только общаться через бота; вход по телефону и коду Telegram в приватном меню, без терминала. В production этого форка сервер публикует только жёсткий allowlist read-only инструментов. Сырой и капризный - включается вручную, **на ваш страх и риск**. [Подробности](docs/userbot.md).
- **Граф контактов Telegram (только чтение)** - понятные карточки с доказательствами держат рядом
  имя, день рождения и текущий возраст, город, учёбу, работу, контакты, полезные факты и связи.
  Обычный sync следит за новым, а возобновляемая перестройка личек может пройти всю доступную историю.
...
- Каждое обычное сообщение падает в дневной markdown-лог дословно. Секретные поля меню, включая ключи userbot, телефон и 2FA, удаляются до обработки и в лог не попадают.
...
- **Приватное подключение в чате** - откройте `/menu`, введите телефон в delete-before-processing шаге и наберите код Telegram на маскированной клавиатуре. Терминал не нужен, секреты не уходят модели.
- **Production только для чтения** - MCP-сервер регистрирует явный allowlist инструментов чтения и поиска плюс read-only проверку статуса входа. Отправка, изменение, удаление, вступление, инвайты, реакции и экспорт ссылок-приглашений отсутствуют независимо от ответа модели.
...
- **Ограниченный граф контактов** - каждый sync делает не больше одного запроса к модели на чат и
  берёт самые новые целые сообщения, которые помещаются в контекст; параллельно идут до трёх чатов.
  Сохранённые курсоры, ссылки на сообщения-источники и Markdown-файл с уточняющими вопросами помогают
  следующим инкрементальным проходам. Отдельная перестройка с backup идёт по личным чатам от старых
  сообщений к новым и не меняет семантику обычного sync; голосовые и видеокружки здесь не анализируются.
...
Модель по умолчанию deepseek-v4-flash, контекст 131k. На Go выходит около $9/мес со всем ($5 модель плюс $4-5 VPS), без наценки; голос едет на бесплатном кредите Deepgram. Списки моделей, лимиты и матрица поиска: [docs/providers.md](docs/providers.md).
...
<details>
<summary><b>v0.3.14 · 09.08.2026 - развернуть последние релизы</b></summary>

### 09.08.2026

#### v0.3.14

- Индикатор «Работаю…» с кнопкой «Стоп» теперь всегда исчезает после ответа - реплай на сообщение Ивы, пока она ещё занята, раньше навсегда оставлял прежний индикатор висеть в чате.
- Упавшие и припаркованные ходы прибирают за собой: осиротевший индикатор удаляется сам, без ручной чистки.
- Все пять статус-лоадеров переехали на анимированные эмодзи (печатающие точки для хода), а грязная установка больше не откатывает исправное обновление - локальные правки сохраняются как recovery bundle.
