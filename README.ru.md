...
- **v0.3.5** 📬 Сообщения в очереди переживают перезапуск, статус «работаю» появляется раньше, цитаты и длинный ввод сохраняют контекст
- **v0.3.5** ⚙️ Смена модели получила живую проверку и откат; ошибки systemd и userbot теперь видны сразу
- **v0.3.5** 🛡️ Дочерние shell-процессы завершаются вместе, самоперезапуск заблокирован, DoS-баги закрыты, добавлены Kimi K3 и рабочие vision-модели
- **v0.3.4** 🧠 Уровень размышлений теперь зависит от модели: привычные кнопки Telegram показывают доступные варианты, а `low` / `medium` / `high` реально доходят до Ollama Cloud и OpenCode Go, как и до подписки OpenAI
- **v0.3.4** ♻️ `/new` теперь сбрасывает только текущий личный чат, диалог в группе или тему форума; `/restart` делает такой же точечный сброс и перезапускает процесс. Eve обновлён до 0.27.8, а `iva reset` остаётся явной аварийной командой для сброса всего агента
- **v0.3.4** 🛡️ При обновлении карточки памяти безопасно объединяются: неизвестные поля и старый текст больше не затираются. API Eve доступен только локально и требует bearer-ключ, секретный файл больше не проваливается в модель, а встроенный rich-post не может выгрузить заметки vault, логи или `.env`

### 23.07.2026

- **v0.3.0** 🎛 Центр управления в чате: вложенное меню `/menu` - модель и размышления, веб-поиск с вводом ключа прямо в диалоге, язык интерфейса (мгновенное переключение ru/en - весь интерфейс теперь двуязычный), онбординг userbot и Google, кроны, скиллы и живая карточка статуса. Плюс 🎭 **тест характера**: 10 вопросов кнопками, четыре оси, 16 архетипов - принятый портрет становится живым характером Ивы со следующего сообщения, а интервью для ядра памяти Ива дистиллирует в `CORE.md` сама
- **v0.3.0** ⏹️ Управление на лету: `/stop` (или кнопка ⏹ на статус-сообщении) обрывает текущий ход прямо из чата, сообщения в занятое время встают в очередь, а у статуса «работаю» появился анимированный лоадер
...
- **v0.2.6** 🧪 **Userbot для личного Telegram (бета)** - Iva читает и отправляет сообщения от вашего *собственного* аккаунта; онбординг целиком в чате (QR, без терминала), анти-бан защита (лимиты FloodWait, рандомизированный темп, circuit-breaker) включена на сервере принудительно. Включается вручную, на свой риск

### 15.07.2026

- **v0.2.5** 🧾 Богатые ответы: таблицы, чек-листы, спойлеры и формулы рендерятся в Telegram нативно (Bot API 10.1), для обычного текста остаётся проверенный путь

### 09.07.2026

- **v0.2.4** 📮 Google Workspace из чата: Gmail, Calendar, Drive, Sheets и Docs через `gws` CLI - инсталлер ставит его сам, ключ регистрируется прямо в диалоге. Плюс провайдер OpenRouter и его честные ошибки: обманчивое «нужен function calling» заменено настоящей причиной

### 06.07.2026

- **v0.2.1-0.2.3** 🤖 Iva работает на вашей подписке ChatGPT (`MODEL_PROVIDER=codex`) - отдельный API-ключ не нужен; документация разложена на 10 страниц с русскими зеркалами и `llms.txt`

### 04.07.2026

- **v0.2.0** 🧠 Память ищет по смыслу: BM25 плюс rerank по связям карточек на любом языке; факты при смене реальности переписываются, строго типизированные карточки; детерминированные гейты против prompt-injection и утечки секретов; опциональный векторный режим одним ключом

### 29.06.2026

- **v0.1.7** 👁️ Iva видит картинки - их описывает vision-модель вашего же провайдера на том же ключе, с OCR; вложение любого типа больше не роняет ход; ночной rollup записывает ваши поправки в CORE: что поправили сегодня, выполняется завтра
